...
class DefaultController extends Controller
{
    ...
    /**
     * Delete the hierarchy item (and all of its page contents) with the given id.
     * The result is returned as a json encoded object.
     *
     * @menuLabel       __HIDDEN__
     * @functionalRight cmsBackendWrite
     ...
     * @return string
     */
    public function actionDeleteHierarchyItemJson($hierarchyItemId = -1)
    {
        $result = [];

        if ($hierarchyItemId == -1) {
            $result ['result'] = 'failed';
            $result ['message'] = 'missing required parameters for action';
        } else {
            /**
            ...
            if ($cmsHierarchyItem) {
                if ($cmsHierarchyItem->id == DefaultController::$ROOT_HIERARCHY_ITEM_ID) {
                    $result ['result'] = 'failed';
                    $result ['message'] = 'the root node cannot be deleted';
                } else {
                    if ($cmsHierarchyItem->delete()) {
                        $result ['result'] = 'success';
                        $result ['message'] = 'hierarchy node has been deleted';
                    } else {
                        $result ['result'] = 'failed';
                        $result ['message'] = 'error while trying to update hierarchy node';
                    }
                ...
                $result ['result'] = 'failed';
                $result ['message'] = 'unknown hierarchy node id given. Deletion failed';
            }
        }
